...
function main() {
    ...
    // TODO we have randomly failing execs
    // e.g  ChildProcessError: Command failed: docker unpause --help | grep -e '^  ' (exited with error code 1)
    ...
    exec("docker --help | grep -e '^  [a-z]' | sed 's/  \\(\\w*\\).*/\\1/'")
        .then(result => {
            ...
            commands.forEach(command => {
                ...
                promises.push(
                    exec(`docker ${command} --help | grep -e '^  '`)
                        .then(result2 => {
